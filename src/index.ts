...
import {
  ...
  DeploymentCreatedWebhookPayload,
  DeploymentStatusCreatedWebhookPayload,
...
async function handleRequest(
  ...
  if (event != null && contentType != null) {
    ...
    let [hookId, hookToken] = (url.pathname + url.search)
      .substring(1)
      .split('/');
    ...
    const result = await fetch(
      `https://discord.com/api/webhooks/${hookId}/${hookToken}`,
      {
        headers: {
    ...
    if (result.ok) {
      return new Response(`disgit successfully executed webhook ${hookId}`, {
        status: 200,
      });
    }

    return new Response(
      `Failed to send webhook ${hookId}: Got ${result.status} from discord: ${await result.text()}`,
      { status: 400 },
    );
  } else {
...
function buildPing(json: PingWebhookPayload, env: BoundEnv): string | null {
  ...
  return buildEmbedBody(
    env,
    `[${name}] ${hook?.type} hook ping received`,
    undefined,
    sender,
    12118406,
    zen,
...
function buildRelease(
  json: ReleaseCreatedWebhookPayload | PrereleaseCreatedWebhookPayload,
  env: BoundEnv,
): string | null {
...
function buildPullDequeue(
  json: PullRequestDequeuedWebhookPayload,
  env: BoundEnv,
): string | null {
  const { pull_request, repository, sender } = json;
...
function buildPull(
  json: PullRequestOpenedWebhookPayload,
  env: BoundEnv,
): string | null {
  const { pull_request, repository, sender } = json;
...
function buildIssueComment(
  json: IssueCommentCreatedWebhookPayload,
  env: BoundEnv,
): string | null {
...
function buildIssue(
  json: IssueCreatedWebhookPayload,
  env: BoundEnv,
): string | null {
  const { issue, repository, sender } = json;
...
function buildPackagePublished(
  json: PackagePublishedWebhookPayload,
  env: BoundEnv,
): string | null {
...
function buildDiscussionComment(
  json: DiscussionCommentCreatedWebhookPayload,
  env: BoundEnv,
): string | null {
...
function buildDiscussion(
  json: DiscussionCreatedWebhookPayload,
  env: BoundEnv,
): string | null {
...
function buildDeleteBranch(
  json: BranchDeletedWebhookPayload,
  env: BoundEnv,
): string | null {
...
function buildCreateBranch(
  json: BranchCreatedWebhookPayload,
  env: BoundEnv,
): string | null {
  const { ref, ref_type, repository, sender } = json;
...
function buildCommitComment(
  json: CommitCommentCreatedWebhookPayload,
  env: BoundEnv,
): string | null {
...
function buildCheck(
  json: CheckRunCompletedWebhookPayload,
  env: BoundEnv,
): string | null {
...
function buildDeploymentStatus(
  json: DeploymentStatusCreatedWebhookPayload,
  env: BoundEnv,
) {
